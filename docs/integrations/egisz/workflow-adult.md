...
### **Доверенное лицо**
*(если окно открыто из карты пациента, имеющего представителя (раздел поручительство))* <br>
Смотреть [Отправка данных о приеме несовершеннолетнего пациента](workflow-child.md)
...
### ***Вкладка «Данные - врач»*** 
   – заполняется данными врача, осуществляющего прием

**Врач, ФИО** 
    – изменяется в выпадающем меню (если активный пользователь авторизован под у/з администратора);

**Врач, СНИЛС №** 
    – значение в поле изменяется и сохраняется в личную информацию врача (при отсутствии значения – оно заполняется вручную и, по кнопке «Сохранить для врача», сохраняется в личную информацию врача);

**Кнопка «Сохранить для врача»** 
    – для сохранения/изменения номера СНИЛС врача из данной формы в базе.

**Должность** – изменяется в выпадающем меню (по умолчанию "Врач-стоматолог"); <br>
 ![](../../images/extracted/ЕГИСЗ/image14.png)
...
**Специальность** 
    – заполняется автоматически, если в настройках ЕГИСЗ вы не выставили галочку «Разрешить специальности врачей» (по умолчанию «Стоматология (код 3)»). 
 
Список специальностей ниже: <br>
 ![](../../images/extracted/ЕГИСЗ/image15.png)

**Врач (сертификат) ФИО** 
    – ФИО врача, указанного в сертификате

**Чек-бокс «Не использовать подпись организации»** 
    - использовать в случае, если вы не хотите подписывать документ от имени организации.

<br>

### ***Вкладка «Данные – пользователь ЕГИСЗ»*** 
   – заполняется личными данными сотрудника, осуществляющего отправку данных о приёме

**Пользователь, ФИО** 
    – изменяется в выпадающем меню (если активный пользователь авторизован под у/з администратора);

**Пользователь, СНИЛС №** 
    – значение в поле изменяется и сохраняется в личную информацию врача/сотрудника, осуществляющего отправку данных.

**Должность** 
    – изменяется в выпадающем меню (по умолчанию "Врач-стоматолог"); 
 
!!! note "Смотрите вкладку «Данные - врач»"
     Список должностей аналогичен списку должностей во вкладке «Данные - врач».

**Специальность** 
    – заполняется автоматически, если в настройках ЕГИСЗ вы не выставили галочку «Разрешить специальности врачей» (по умолчанию «Стоматология (код 3)»). 

!!! note "Смотрите вкладку «Данные - врач»"
     Список специальностей аналогичен списку специальностей во вкладке «Данные - врач».

**Кнопка «Задать по умолч.»** 
    – позволяет сохранить указанного врача/сотрудника, как осуществляющего отправку данных, по умолчанию.  <br>
    Заданный врач/сотрудник будет отображаться осуществляющим отправку и для остальных пациентов. 

!!! note "Выбор можно изменить"
     Выбрать другого врача/сотрудника, затем нажать кнопку «Задать по умолч.»
...
!!! info "Информация"
    *Поле «Комментарий» не обязательное, поэтому заполнять его или нет решает сам врач.*
